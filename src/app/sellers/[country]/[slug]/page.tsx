...
async function fetchPublicSeller(
  ...
  try {
    const res = await fetch(
      `${baseUrl}/marketplace/sellers/${encodeURIComponent(idOrSlug)}`,
      { next: { revalidate: 60 } },
    );
...
export async function generateMetadata({
  ...
  const seller = await fetchPublicSeller(slug);
  const readableCountry = toTitle(country);
  const displayName = seller?.name ?? toTitle(slug);

  const title = `${displayName} · Verified ${readableCountry} Supplier on Procur`;
  const description = seller?.description
    ? `${seller.description.slice(0, 155)}`
    : `Browse verified produce and quotes from ${displayName}, a ${readableCountry} supplier on Procur. Create a free buyer account to message and order directly.`;

  const canonical = `/sellers/${country}/${slug}`;
  ...
  return {
    ...
    openGraph: {
      ...
      type: "profile",
      url: canonical,
      images: seller?.header_image_url
        ? [{ url: seller.header_image_url }]
        : seller?.logo_url
          ? [{ url: seller.logo_url }]
          : undefined,
    },
...
export default async function PublicSellerProfile({ params }: SellerPageProps) {
  ...
  const expectedCountry = createCountrySlug(seller.location);
  if (country !== expectedCountry) notFound();
