...
import SellersGridClient from "./SellersGridClient";
...
async function fetchSellersPage(
  countryCode: string | null,
): Promise<{ sellers: PublicSeller[]; total: number }> {
  const baseUrl =
    process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3000/api/v1";
  const params = new URLSearchParams({
    limit: String(INITIAL_PAGE_LIMIT),
    page: "1",
  });
  ...
  try {
    const res = await fetch(`${baseUrl}/marketplace/sellers?${params}`, {
      headers,
      next: { revalidate: 60 },
    });
    if (!res.ok)
      return { sellers: FALLBACK_SELLERS, total: FALLBACK_SELLERS.length };
    const data = await res.json();
    const sellers = data?.sellers as PublicSeller[];
    const total = typeof data?.total === "number" ? data.total : 0;
    return Array.isArray(sellers) && sellers.length > 0
      ? { sellers, total }
      : { sellers: FALLBACK_SELLERS, total: FALLBACK_SELLERS.length };
  } catch {
    return { sellers: FALLBACK_SELLERS, total: FALLBACK_SELLERS.length };
  }
}
...
export default async function SellersPage() {
  ...
  const countryCode = cookieStore.get("country_code")?.value ?? null;
  const { sellers, total } = await fetchSellersPage(countryCode);
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 13, color: "#8a9e92", margin: 0 }}>
              {total || sellers.length} verified suppliers on Procur
            </p>
          </div>
          <Link
            href="/browse"
